...
//     try {
//       const response = await fetch("http://127.0.0.1:8000/api/admin/api/admin/", {
//         method: "POST",
//         headers: {
...
//           type: "success",
//         });
//         setCreatedRestaurantId(data.id); // Lưu ID nhà hàng mới
//       } else {
...
//     try {
//       const response = await fetch(`http://127.0.0.1:8000/api/admin/api/admin/${createdRestaurantId}/menu`, {
//         method: "POST",
//         headers: {
//           "Content-Type": "application/json",
//           "Authorization": `Bearer ${token}`,
...
//     try {
//       const menuItemWithRestaurantId = {
//         ...newMenuItem,
...
export default function CreateRestaurant() {
  ...
  const handleSelectChange = (name: string, value: string) => {
    ...
    // Log để debug
    console.log(`Select field "${name}" updated with value: "${value}"`);
  };
  ...
  const validateRestaurantForm = () => {
    ...
    for (const field of requiredFields) {
      if (!formData[field]) {
        setAlert({
          message: `Vui lòng nhập đầy đủ thông tin bắt buộc: ${field}`,
          type: "error",
        });
        return false;
      }
    ...
    // Log toàn bộ dữ liệu form trước khi submit để debug
  ...
  const handleCreateRestaurant = async () => {
    ...
    try {
      // ✅ Khai báo rõ ràng trước khi tạo object restaurantData
      const priceRange = (document.getElementById("price_range") as HTMLInputElement)?.value.trim() || "";
      const targetAudience = (document.getElementById("target_audience") as HTMLInputElement)?.value.trim() || "";
      ...
      console.log("Sending data to API:", restaurantData);
      ...
      try {
        ...
        setAlert({
          message: `Lỗi: Không thể xử lý phản hồi từ server`,
          type: "error",
        });
        return;
      }
      ...
      if (response.ok) {
        ...
        setAlert({
          message: `Lỗi: ${data.detail || "Không thể tạo nhà hàng"}`,
          type: "error",
        });
      }
    } catch (error) {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                ...
                <Input
                  id="price_range"
                  name="price_range"
                  placeholder="Nhập khoảng giá (VD: 100.000 - 300.000)"
                  value={formData.price_range}
                  onChange={handleInputChange}
                />
              </div>
              ...
              <div className="space-y-2">
                <Label htmlFor="category">Loại hình quán ăn <span className="text-red-500">*</span></Label>
                <Input
                  id="category"
                  name="category"
                  placeholder="Nhập loại hình quán ăn (VD: Nhà hàng, Quán cà phê, Đồ ăn nhanh...)"
