...
def hybrid_recommend(user_id: int, db: Session, top_n=5):
    try:
        ...
        # Content-based luôn được áp dụng
        result_cb = content_based_recommend(user.__dict__, df_restaurants, top_n=10)
        
        # Nếu người dùng không có đủ đánh giá, chỉ trả về content-based
        if review_count < 3:
            return safe_json_serializable(result_cb)
            
        # Áp dụng cả hai phương pháp và kết hợp kết quả
        try:
            # Get collaborative filtering results
            result_cf_objs = collaborative_filter_recommend(user_id, db, top_n=10)
            ...
            # Thêm cột điểm cho từng phương pháp
            df_cb = result_cb.copy()
            df_cb["cb_score"] = df_cb["score_total"]
            
            # Xử lý các cột thiếu trong df_cf
            for col in df_cb.columns:
                if col not in df_cf.columns:
                    df_cf[col] = 0
            
            # Tính điểm CF dựa trên cosine similarity và xếp hạng
            if "similarity" in df_cf.columns:
                df_cf["cf_score"] = df_cf["similarity"]
            else:
                # Nếu không có sẵn điểm similarity, tạo xếp hạng dựa trên vị trí
                df_cf["cf_score"] = np.linspace(1.0, 0.1, len(df_cf))
                
            # Kết hợp 2 dataframe, ưu tiên giữ lại nhà hàng có trong cả 2 phương pháp
            df_combined = pd.merge(df_cb, df_cf[["id", "cf_score"]], on="id", how="outer")
            
            # Xử lý missing values
            df_combined["cb_score"] = df_combined["cb_score"].fillna(0)
            df_combined["cf_score"] = df_combined["cf_score"].fillna(0)
            
            # Tính điểm hybrid: kết hợp cả hai phương pháp với trọng số tùy chỉnh
            # Người dùng càng có nhiều đánh giá, CF càng được coi trọng
            cf_weight = min(0.7, 0.3 + (review_count * 0.05))  # Tăng dần theo số lượng review, tối đa 0.7
            cb_weight = 1.0 - cf_weight
            
            df_combined["hybrid_score"] = (cb_weight * df_combined["cb_score"]) + (cf_weight * df_combined["cf_score"])
            
            # Sắp xếp theo điểm hybrid và trả về kết quả
            result = df_combined.sort_values("hybrid_score", ascending=False).head(top_n)
            return safe_json_serializable(result)
